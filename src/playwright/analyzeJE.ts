 ...
 * ─────────────────────────────────────────────────────────────────────────────
 * Run:  npm run pw:analyze:je                 (newest captured/je-live-*.jsonl)
 *       npm run pw:analyze:je -- <file.jsonl>
 *
 ...
 * Writes captured/je-endpoints.json.
 */
...
function newestLive(): string {
  const files = readdirSync('captured')
    .filter((f) => f.startsWith('je-live-') && f.endsWith('.jsonl'))
    .sort();
  if (!files.length) throw new Error('No captured/je-live-*.jsonl. Run `npm run pw:capture:je` first.');
...
function main() {
  ...
  if (authFails.length > recs.length / 2) {
    ...
    console.log('   `npm run pw:login`, then `npm run pw:capture:je` again.\n');
  }
  ...
  writeFileSync('captured/je-endpoints.json', JSON.stringify(out, null, 2));
  console.log('\n📝 Wrote captured/je-endpoints.json');
}
